...
void SparseVector::at(int index) const
{
    if((index<0)||(index>=getSize()))
    {
	std::cerr<< "at" <<index<<std::endl;        
	throw std::out_of_range("index is either <0 or >= Size");
    	
    }
}

void SparseVector::cv(int val) const
{
    if(val<0)
    {
